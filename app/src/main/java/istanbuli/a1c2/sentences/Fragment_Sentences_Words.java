...
public class Fragment_Sentences_Words extends Fragment {
 ...
 {
     String[] speak=lv_list_words.getAdapter().getItem(id_start).toString().split(" ");
     st.setspeake(getActivity(),speak[0]);
 }
    ...
    public void LoadListLesson() {
        ...
        if(!myPrefs.getSentence_Titel().equals("Alphabet"))
        {
            b = "Tr_Tbl_Sentence_Dictionary";

            areaTyp = "SELECT *  FROM " + b + " where Cat_Id=" + lesson.trim();
        ...
        else
        {
            ...
            if (cr.moveToFirst()) {
                do {
                    ...
                    fa_words.add(cr.getString(cr.getColumnIndex("Fa")));
                    en_words.add(cr.getString(cr.getColumnIndex("Sample")));
                } while (cr.moveToNext());
            }
        }
    ...
    public void ClickOnWordsList4(View v)
    {
        ...
        {
            String[] speak=child.getText().toString().split(" ");
            st.setspeake(getActivity(),speak[0]);
        }
